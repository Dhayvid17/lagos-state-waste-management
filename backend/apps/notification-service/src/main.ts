...
async function bootstrap(retryCount = 0) {
  ...
  try {
    ...
    const port = parseInt(config.get<string>('NOTIFICATION_SERVICE_PORT') ?? '3005', 10);
    ...
    if (process.env.NODE_ENV !== 'prod') {
      const swaggerConfig = new DocumentBuilder()
        .setTitle('Lagos Waste — Notification Service')
      ...
      logger.log(`📚 Swagger: http://localhost:${port}/api/notifications/docs`);
    }
    ...
    logger.log(`📡 NATS listeners active — subscribed to all platform events`);
    logger.log(`📧 Resend ready`);
    logger.log(`📱 Africa's Talking SMS ready`);
    logger.log(`🔔 Firebase Push ready`);
  } catch (error) {
