...
async function bootstrap(retryCount = 0) {
  const logger = new Logger('CollectorService');
  ...
  try {
    ...
    const port = parseInt(config.get<string>('COLLECTOR_SERVICE_PORT') ?? '3007', 10);
    ...
    if (process.env.NODE_ENV !== 'prod') {
      const swaggerConfig = new DocumentBuilder()
        .setTitle('Lagos Waste — Collector Service')
        .setDescription('Collector Assignment & GPS Tracking API')
        .setVersion('1.0')
        .addBearerAuth()
        .build();

      const document = SwaggerModule.createDocument(app, swaggerConfig);
      SwaggerModule.setup('api/collector/docs', app, document);
      logger.log(`📚 Swagger: http://localhost:${port}/api/collector/docs`);
    }
    ...
    logger.log(`🚀 Collector Service running on http://localhost:${port}/api`);
    logger.log(`📍 GPS tracking active — 30s ping interval`);
    logger.log(`🗑️  NDPA cleanup cron active — 90 day retention`);
    logger.log(`🗺️  Live collector map available for admins`);
  } catch (error) {
    logger.error('❌ Collector Service failed to start');
